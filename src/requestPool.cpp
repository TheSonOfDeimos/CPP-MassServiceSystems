...
SourseOfRequest::SourseOfRequest(int source_id, double alpha, double betta)
    : id_(source_id),
    time_(0),
    counter_(0),
    alpha_(alpha),
...
alpha_(std::move(source.alpha_)),
betta_(std::move(source.betta_))
{
}
...
SourseOfRequest::~SourseOfRequest()
{
    ...
    std::unique_ptr< Request > SourseOfRequest::release()
    {
        auto t = generate();
        time_ += t;
        std::unique_ptr< Request > ptr = std::make_unique< Request >(Request(time_, 0, 0, 0, id_, 0));
    ...
    double SourseOfRequest::generate()
    {
        //time_ += 1 / static_cast< double >(std::rand() % 10 + 1);
        //return {time_, 0, 0, 0, id_, 0};
        double res = ((double)rand() / (double)RAND_MAX) * (betta_ - alpha_) + alpha_;
        return res;
        //return  1 / static_cast< double >(std::rand() % 10 + 1);
    }
...
void SourseOfRequest::setRandom(double alpha, double betta)
{
    alpha_ = alpha;
    betta_ = betta;
}
...
SourcePool::SourcePool(int source_val, double alpha, double betta)
: counter_(0)
    {
        for (int i = 0; i < source_val; ++i)
        {
            pool_.emplace_back(i, alpha, betta);
        }
    }
